...
const DcotorsViewBar = ({isOpen, setIsOpen,setSelectedUser,allConversation,currentChat,SetCurrentChat  ,user,SetUser ,sendDataToParent}) => {
    ...
    return (
        <>
            <div className={` flex ${!isOpen? "w-full md:w-[30%]":"hidden md:block "} flex-col h-[75vh] rounded-md border-[1px] border-gray-200 mt-3`}>
                ...
                <div className="cardwrapper cursor-pointer flex flex-col gap-3 mt-3 ">
                    {
                        ...
                        allConversation?.map((item,i) => (
                            ...
                            <ChatMessages  SetCurrentChat={SetCurrentChat} sendDataToParent={sendDataToParent}  user={user} SetUser={SetUser} chatsMembers={item} currentUser={userId} />
                            </div>
                        ))
                    }
